...
async def warning(client, message):
    ...
    if rule_breaks == 0:
        ...
        violation_text = "Please do not mention other users in this group." if "@" in message.text else (
            "Third-party links are not allowed in this group." if "http" in message.text or "https" in message.text
            else "Telegram links are not allowed in this group."
        )
    ...
    await client.send_message(chat_id, warning_text, reply_to_message_id=message.message_id)

    # Log violation to LOG_CHANNEL
    log_message = (
        f"**Rule Violation**\n\nChat ID: {chat_id}\nUser ID: {user_id}\nUsername: {message.from_user.mention}\n"
        f"Rule Broken: {warning_text}\nDate: {time.strftime('%Y-%m-%d')}\nTime: {time.strftime('%H:%M:%S')}"
    )
    ...
    if duration is not None:
        ...
        await client.send_message(chat_id, "Your ban has ended. Please refrain from violating group rules.")
