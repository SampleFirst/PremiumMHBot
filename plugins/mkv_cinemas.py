from pyrogram import Client, filters
from pyrogram.types import InlineKeyboardButton, InlineKeyboardMarkup
import requests
from bs4 import BeautifulSoup
from io import BytesIO
from info import ADMINS 


url_list = {}
api_key = "3269bf2096dd8aec64d201398176d3db93ce68db"


@Client.on_message(filters.command("mkvcinemas"))
def mkvcinemas(client, message):
    query = message.text.split(maxsplit=1)
    if len(query) == 1:
        message.reply_text("Please provide a movie name to search.")
        return
    query = query[1]
    search_results = message.reply_text("Processing...")
    movies_list = search_movies(query)
    if movies_list:
        keyboards = []
        for movie in movies_list:
            keyboard = InlineKeyboardButton(movie["title"], callback_data=movie["id"])
            keyboards.append([keyboard])
        reply_markup = InlineKeyboardMarkup(keyboards)
        search_results.edit_text('Search Results...', reply_markup=reply_markup)
    else:
        search_results.edit_text('Sorry 🙏, No Result Found!\nCheck If You Have Misspelled The Movie Name.')

@Client.on_callback_query()
def movie_result(client, callback_query):
    query = callback_query
    s = get_movie(query.data, is_admin(callback_query.from_user.id))
    response = requests.get(s["img"])
    img = BytesIO(response.content)
    callback_query.message.reply_photo(photo=img, caption=f"🎥 {s['title']}")
    link = ""
    links = s["links"]
    for i in links:
        link += f"🎬 {i}\n{links[i]}\n\n"
    caption = f"⚡ Fast Download Links :-\n\n{link}"
    if len(caption) > 4095:
        for x in range(0, len(caption), 4095):
            callback_query.message.reply_text(text=caption[x:x + 4095])
    else:
        callback_query.message.reply_text(text=caption)


def search_movies(query):
    movies_list = []
    website = requests.get(f"https://mkvcinemas.dev/?s={query.replace(' ', '+')}")
    if website.status_code == 200:
        website = website.text
        website = BeautifulSoup(website, "html.parser")
        movies = website.find_all("a", {'class': 'ml-mask jt'})
        for index, movie in enumerate(movies):
            movies_details = {}
            if movie:
                movies_details["id"] = f"link{index}"
                movies_details["title"] = movie.find("span", {'class': 'mli-info'}).text
                url_list[movies_details["id"]] = movie['href']
                movies_list.append(movies_details)
    return movies_list


def get_movie(query, is_admin):
    movie_details = {}
    movie_page_link = requests.get(f"{url_list[query]}")
    if movie_page_link.status_code == 200:
        movie_page_link = movie_page_link.text
        movie_page_link = BeautifulSoup(movie_page_link, "html.parser")
        title = movie_page_link.find("div", {'class': 'mvic-desc'}).h3.text
        movie_details["title"] = title
        img = movie_page_link.find("div", {'class': 'mvic-thumb'})['data-bg']
        movie_details["img"] = img
        links = movie_page_link.find_all("a", {'rel': 'noopener', 'data-wpel-link': 'internal'})
        final_links = {}
        for i in links:
            if is_admin:
                final_links[i.text] = i['href']
            else:
                url = f"https://urlshortx.com/api?api={api_key}&url={i['href']}"
                response = requests.get(url)
                link = response.json()
                final_links[i.text] = link['shortenedUrl']
        movie_details["links"] = final_links
    return movie_details
