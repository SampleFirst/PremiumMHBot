...
# Function to momo text and links from a website
async def momo_text_links(website_link):
    try:
        ...
        links = [f"* [{link}]({link})" for link in soup.find_all('a', href=True) if link['href'].startswith('http')]
...
async def handle_momo_command(Client, message):
    try:
        ...
        extracted_text = await momo_text_links(website_link)
