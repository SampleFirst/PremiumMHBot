...
def find_movie_one(client, message):
    ...
    movies_list = search_movies(query, "skymovieshd")
    if movies_list:
        keyboards = []
...
def find_movie_two(client, message):
    query = message.text.split(maxsplit=1)
    if len(query) == 1:
        message.reply_text("Please provide a movie name to search.")
        return
    query = query[1]
    search_results = message.reply_text("Processing...")
    movies_list = search_movies(query, "mkvcinemas")
    if movies_list:
        keyboards = []
        for movie in movies_list:
            keyboard = InlineKeyboardButton(movie["title"], callback_data=movie["id"])
...
def movie_result(client, callback_query):
    ...
    try:
        ...
        query.message.reply_text(f"An error occurred: {str(e)}")

def search_movies(query, website):
    movies_list = []
    movies_details = {}
    try:
        if website == "skymovieshd":
            ...
            if website.status_code == 200:
                ...
                movies = website.find_all("a", {'class': 'text-white'})
                for movie in movies:
                    if movie:
                        movies_details["id"] = f"link{movies.index(movie)}"
                        movies_details["title"] = movie.text.strip()
                        url_list[movies_details["id"]] = movie['href']
                    movies_list.append(movies_details)
                    movies_details = {}
        elif website == "mkvcinemas":
