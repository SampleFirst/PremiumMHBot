...
@Client.on_message(filters.command("latestupdate") & filters.user(ADMINS))
async def handle_latest_update(client, message):
...
async def handle_my_update(client, message):
