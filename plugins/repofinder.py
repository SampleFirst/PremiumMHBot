...
async def repo(client, message):
    try:
        ...
        if len(command_parts) > 1:
            ...
            headers = {"Authorization": "Bearer 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"}
                ...
                if "items" in data and len(data["items"]) > 0:
                    ...
                    if branches and len(branches) > 0:
                        message_file = (
                            ...
                            "\n".join([
                                f"**[{branch['name']} ZIP]({repo_url}/archive/refs/heads/{branch['name']}.zip)**"
                                for branch in branches
                            ])
                        )
                    else:
                        message_file = f"Repo Zip File: ({repo_url}/archive/refs/heads/{default_branch}.zip)"
