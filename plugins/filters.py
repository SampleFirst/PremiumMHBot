...
async def add_filter(client, message):
    ...
    if len(args) < 2:
        await message.reply_text("Command incomplete. Please provide the filter content.", quote=True)
        return
    ...
    if len(extracted) >= 2 and not message.reply_to_message:
        ...
        fileid = None
        if not reply_text:
            await message.reply_text("You cannot have buttons alone. Please provide some text to go with it!", quote=True)
            return
...
async def delete_all_confirm(client, message):
    userid = message.from_user.id if message.from_user else None
    if not userid:
        return await message.reply(f"You are an anonymous admin. Use /connect {message.chat.id} in PM")
    chat_type = message.chat.type
