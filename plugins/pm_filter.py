...
async def select_language(bot, query):
    ...
    btn = [
        ...
        [
            ...
            InlineKeyboardButton("Bhojpuri", callback_data=f"lang#{userid}#bho")
        ],
        [
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        await query.message.edit_text(f"<b>Fetching files for your query '{keyword}' on DB... Please wait...⌛️</b>")
        files, total = await get_bad_files(keyword)
        await query.message.edit_text(f"<b>Found {total} files for your query '{keyword}'! File deletion process will start in 5 seconds!⚠️</b>")
        ...
        async with lock:
            try:
                for file in files:
                    ...
                    deleted += 1
                    if deleted % 20 == 0:
                        await query.message.edit_text(f"<b>Process started for deleting files from DB. Successfully deleted {str(deleted)} files from DB for your query '{keyword}'! Please wait...⌛️</b>")
            except Exception as e:
                logger.exception(e)
                await query.message.edit_text(f"Error: {e}")
            else:
                await query.message.edit_text(f"<b>Process completed for file deletion! Successfully deleted {str(deleted)} files from DB for your query '{keyword}'.✅</b>")
